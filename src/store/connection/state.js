export default {
    ...
    getters: {
        ...
        nftsModels: (state, getters) =>  getters.nfts.filter(nft => nft.model != "metadata" && nft.uri != "" && nft.json),
        nftsMetadata: (state, getters) =>  state.nftsAddresses.reduce((acc, nft) => {
            if(state.nftsMetadataMapped[nft]){
                return [...acc, state.nftsMetadataMapped[nft]]
            }
            return acc
        }, []),
        nftsMetadataMapped: (state) => state.nftsMetadataMapped,
